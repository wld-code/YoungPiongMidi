...
pitch detection, the note-stabilization state machine), per the project
spec's section 17: "DSP algorithms should be testable without the ESP32
when possible." These build and run on your Mac/Linux machine with a
plain C compiler - no ESP-IDF, no board, no simulator.
...
| `test_note_state_machine.c` | `components/voice_midi/note_state_machine.c` | the spec's own anti-flicker requirement (60 frames of +/-15-cent jitter around a held note -> zero events), Note On only after `YP_NOTE_MIN_STABLE_FRAMES`, Note Off only after `YP_NOTE_RELEASE_FRAMES` (and *not* on a shorter dropout that recovers), a note change withheld until `YP_NOTE_MIN_DURATION_MS` has elapsed, low confidence never triggering anything, and `yp_level_to_velocity`'s bounds/clamping/monotonicity for both curves |
...
`rms.c`, `envelope.c`, `yin.c`, `voice_midi.c` and `note_state_machine.c`
have zero ESP-IDF dependency by design - they only ever include
